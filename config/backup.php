...
return [
    ...
    'backup' => [
         ...
         * the backups.
         */
        'name' => env('BACKUP_NAME', 'surreytuning-central'),
        ...
        'source' => [
             ...
             */
            /*
             * Only the central database is backed up by the standard
             * `backup:run` command. Per-tenant databases are backed up
             * separately by the `backup:tenants` command (see
             * app/Console/Commands/BackupTenantDatabases.php), which
             * overrides this key to the dynamic `tenant` connection for
             * each tenant in turn. Do NOT add `sqlite`/`default` here —
             * that would try to dump the test/in-memory connection.
             */
            'databases' => [
                'mysql',
            ],
