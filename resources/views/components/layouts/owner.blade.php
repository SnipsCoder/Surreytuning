            ...
            <aside class="hidden lg:flex lg:flex-col flex-shrink-0 w-64 bg-[#1e293b] border-r border-white/5">
                <div class="flex items-center h-16 px-5 bg-black border-b border-white/5 flex-shrink-0">
                    @if ($settings && ($settings->portal_logo || $settings->logo_dark))
                        <img src="{{ route('branding.logo') }}" alt="{{ $brandName }}" class="h-8 max-w-[148px] object-contain">
                    @else
                ...
                <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-6">
                    @php
                        ...
                        // Restrict the sidebar for Tuners — they only see the items needed to process file requests.
                        // Everything else (Configuration, Account, and owner-only Operations/Tools items) is hidden
                        ...
                        if (auth()->user()?->role === \App\Enums\UserRole::Tuner) {
                            ...
                            $navSections = collect($navSections)
                                ->map(fn ($links) => array_values(array_filter(
                                    $links,
                                    fn ($link) => in_array($link['route'], $tunerAllowedRoutes, true),
                                )))
