...
@section('content')
    <div class="mb-6 text-center">
        ...
        <p class="mt-1 text-sm text-gray-400">Apply to become an authorised Surrey Tuning dealer</p>
    </div>
    ...
    <form method="POST" action="{{ route('apply.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="company_name" value="{{ old('company_name') }}" required
                class="block w-full rounded-md border-gray-600 bg-gray-800 text-gray-100 text-sm shadow-sm focus:border-[#e63012] focus:ring-[#e63012]">
        </div>
        ...
        <div>
            ...
            <textarea name="message" rows="3"
                class="block w-full rounded-md border-gray-600 bg-gray-800 text-gray-100 text-sm shadow-sm focus:border-[#e63012] focus:ring-[#e63012]">{{ old('message') }}</textarea>
        </div>
        ...
        <label class="flex items-start gap-3">
            <input type="checkbox" name="terms_accepted" value="1" @checked(old('terms_accepted'))
                class="mt-0.5 rounded border-gray-600 text-[#e63012] focus:ring-[#e63012]">
            <span class="text-sm text-gray-300">I agree to the terms and conditions</span>
        </label>

        <button type="submit" class="w-full px-4 py-2 rounded-md bg-[#e63012] text-white text-sm font-medium hover:bg-[#c92a0f]">
            Submit Application
        </button>
    </form>
@endsection
