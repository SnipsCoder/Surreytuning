    ...
    <x-data-table :headers="['Name', 'Vehicle Type', 'Price (Net)', 'Turnaround', 'Active', '']">
        @forelse ($fileStages as $fileStage)
            <tr class="hover:bg-gray-50 dark:hover:bg-[#1a1a1a]/60">
                ...
                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $fileStage->turnaround_hours }}h</td>
                ...
                <td class="px-4 py-3 text-sm text-right space-x-3">
                    <button type="button" x-on:click="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'edit-file-stage-{{ $fileStage->id }}' }))"
