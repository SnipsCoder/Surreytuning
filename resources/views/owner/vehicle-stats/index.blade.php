<x-layouts.owner>
    ...
    <form method="GET" action="{{ route('vehicle-stats.index') }}" class="grid grid-cols-4 gap-4 mb-4">
        <div>
        ...
        @forelse ($vehicleStats as $vehicleStat)
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                ...
                <td class="px-4 py-3 text-sm text-right space-x-3">
                    <a href="{{ route('vehicle-stats.edit', $vehicleStat) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Edit</a>
                    <form method="POST" action="{{ route('vehicle-stats.destroy', $vehicleStat) }}" class="inline" onsubmit="return confirm('Delete this vehicle stat?')">
                        @csrf
                        @method('DELETE')
