@php
    // Drives the live show/hide of the VAT toggle below. Mirror the select's default:
    // when nothing is chosen yet (new product) the first case (File Credits) is selected.
    $selectedPaymentType = old('payment_type', $product?->payment_type?->value)
        ?? \App\Enums\ProductPaymentType::cases()[0]->value;
    $fileCreditsValue = \App\Enums\ProductPaymentType::FileCredits->value;
@endphp

<div x-data="{ paymentType: '{{ $selectedPaymentType }}' }" class="space-y-4">
    <div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <select name="payment_type" x-model="paymentType" required
                class="mt-1 block w-full rounded-md border-gray-300 dark:border-[#2a2a2a] dark:bg-[#0d0d0d] dark:text-gray-100 text-sm shadow-sm">
    ...
    <div class="flex items-center gap-6">
        {{-- File Credits products are the dealer's own products, so no VAT applies: hide the
             toggle and force vat_applicable off on save. Every other payment type keeps the
             real checkbox. Using x-if (not x-show) guarantees exactly one vat_applicable input
             is ever in the DOM, so an unticked-but-hidden checkbox can't silently post. --}}
        <template x-if="paymentType === '{{ $fileCreditsValue }}'">
