    ...
    <x-data-table :headers="['Name', 'Email', 'Role', 'Status', '']">
        @forelse ($users as $user)
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $user->full_name }}</td>
                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $user->email }}</td>
                <td class="px-4 py-3 text-sm">
                    <x-status-badge :status="$user->role->label()" colour="bg-blue-100 text-blue-800" />
                </td>
                <td class="px-4 py-3 text-sm">
                ...
                <td class="px-4 py-3 text-sm text-right">
                    @if ($user->status === 'active' && auth()->user()->role->value === 'dealer_owner' && $user->id !== auth()->id())
                        <form method="POST" action="{{ route('client.portal-users.destroy', $user) }}" class="inline" onsubmit="return confirm('Remove this user?')">
                            @csrf
                            @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No users found.</td>
            </tr>
        @endforelse
    </x-data-table>

    <x-modal id="invite-user" title="Invite User">
        <form method="POST" action="{{ route('client.portal-users.invite') }}" class="space-y-4">
            @csrf
