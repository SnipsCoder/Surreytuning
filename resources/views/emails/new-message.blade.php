...
<x-emails.layout :subject="'New Message on File Request ' . $ref">
    ...
    <p>
        <strong>From:</strong> {{ $threadMessage->sender?->full_name ?? 'System' }}<br>
        <strong>Sent:</strong> {{ $threadMessage->created_at->format('d/m/Y H:i') }}
    </p>
    <blockquote style="border-left: 3px solid {{ $brandColour }}; margin: 16px 0; padding: 8px 16px; color: #4b5563; background: #f9fafb;">
        {{ \Illuminate\Support\Str::limit($threadMessage->body, 300) }}
    </blockquote>
